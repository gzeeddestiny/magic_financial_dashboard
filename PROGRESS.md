...
> อัพเดตล่าสุด: 2026-04-29 (Bangkok) — Light theme redesign + auth/cron/logging fixes
...
### Light Theme Redesign (2026-04-29)
| สิ่งที่ทำ | ไฟล์ | หมายเหตุ |
|-----------|------|----------|
| Complete light theme overhaul | `src/app/globals.css` | เปลี่ยนจาก dark purple/black → light white/purple — `--background` เป็น near-white, `--foreground` เป็น ink-dark, `--card` เป็น pure white, `--primary` ยังเป็น brand purple |
| Glassmorphism อัพเดต | `src/app/globals.css` | `.glass-card` เปลี่ยนเป็น frosted white (`oklch(1 0 0 / 70%)` + blur + saturate) แทน dark glass |
| กราฟ brand colors ปรับ lightness | `src/app/globals.css` | `--color-profit/loss/warning` ลด lightness ลงให้อ่านได้บน white bg; chart palette ปรับทั้งชุด |
| Background gradient | `src/app/globals.css` | เพิ่ม radial gradient fixed attachment (soft purple wash ที่มุม 0%/100%) แทน solid dark bg |
| Badge/status text colors | `anomaly-table.tsx`, `project-ranking.tsx`, `project-table.tsx`, `revenue-expense-trend.tsx`, `net-cash-outlook.tsx`, `ar-installment-table.tsx`, `error.tsx` | เปลี่ยน `-400` variants → `-700` variants ทั้งหมด (green/red/amber/purple/emerald) เพื่อ contrast บน light bg |
| AR table border classes | `src/components/dashboard/ar-installment-table.tsx` | เปลี่ยนจาก `border-white/10 bg-white/5` → `border-border bg-card/60` ให้ใช้ CSS variables แทน hardcode dark colors |
| Scrollbar & selection styles | `src/app/globals.css` | Scrollbar thumb เป็น purple tint; เพิ่ม `::selection` (purple wash) |

### Security & Ops Fixes (2026-04-22 ~ 04-28)
| สิ่งที่ทำ | ไฟล์ | หมายเหตุ |
|-----------|------|----------|
| ALLOWED_EMAILS sign-in allowlist | `src/app/api/auth/[...nextauth]/route.ts` | `signIn` callback เช็ค `process.env.ALLOWED_EMAILS` (comma-separated) — reject login ถ้า email ไม่อยู่ใน list |
| Log Sheets API errors | `src/lib/google/sheets.ts` | เพิ่ม `console.error` แทนการ swallow error เงียบ — ช่วย debug production issues |
| ลบ sync-bl-status cron จาก vercel.json | `vercel.json` | Vercel Hobby plan รองรับ cron เดียว — ย้าย `*/5 * * * *` sync ไปรันที่ **cron-job.org** แทน (endpoint: `/api/cron/sync-bl-status`) |
| USER_GUIDE.md | `USER_GUIDE.md` | เพิ่ม user guide ครอบคลุม: login, 4 dashboard pages, sheet workflows, troubleshooting |
...
1. **Auth bypass in dev** — dev bypass re-added ใน proxy.ts (NODE_ENV=development → skip auth) ⚠️ ต้องลบก่อน production deploy — ALLOWED_EMAILS allowlist ใน NextAuth signIn callback ป้องกัน production แล้ว แต่ยังต้องลบ dev bypass ออก
...
| Task 9: Light Theme Redesign | ✅ เสร็จ | globals.css + component badge colors อัพเดตทั้งหมดสำหรับ light theme |
| Task 10: ALLOWED_EMAILS allowlist | ✅ เสร็จ | NextAuth signIn callback — ตั้ง env `ALLOWED_EMAILS` ใน Vercel ก่อน production |
| Task 11: sync-bl-status cron → cron-job.org | ✅ เสร็จ | ออกจาก vercel.json แล้ว — ต้องตั้ง cron ที่ cron-job.org ชี้ไป `POST /api/cron/sync-bl-status` |
...
15. **env vars** — เก็บ manual (copy .env.local ข้ามเครื่อง) ไม่ได้ใช้ Doppler; ต้องตั้ง `ALLOWED_EMAILS` ใน Vercel สำหรับ production
